import { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { useMediaQuery, useWhatsApp } from "@/hooks";
import { BREAKPOINTS } from "@/lib/constants/config";

/* ============================================
   FloatingWhatsApp Component (Molecule)
   ============================================
   Botón flotante de WhatsApp que aparece después de 3 segundos
   con mensaje contextual y diseño responsivo.
   ============================================ */

const FloatingWhatsApp = () => {
  const [isVisible, setIsVisible] = useState(false);
  const [showTooltip, setShowTooltip] = useState(false);
  const isMobile = useMediaQuery(`(max-width: ${BREAKPOINTS.md}px)`);
  const { sendGeneralContact } = useWhatsApp();

  // Mostrar el botón después de 3 segundos
  useEffect(() => {
    const timer = setTimeout(() => {
      setIsVisible(true);
      // Mostrar tooltip automáticamente después de aparecer (solo desktop)
      if (!isMobile) {
        setTimeout(() => {
          setShowTooltip(true);
          // Ocultar tooltip después de 5 segundos
          setTimeout(() => setShowTooltip(false), 5000);
        }, 500);
      }
    }, 3000);

    return () => clearTimeout(timer);
  }, [isMobile]);

  const handleClick = () => {
    sendGeneralContact();
  };

  return (
    <AnimatePresence>
      {isVisible && (
        <motion.div
          className="fixed bottom-8 right-8 z-50"
          initial={{ opacity: 0, x: 100 }}
          animate={{ opacity: 1, x: 0 }}
          exit={{ opacity: 0, x: 100 }}
          transition={{ duration: 0.5, ease: "easeOut" }}
        >
          {/* Tooltip */}
          <AnimatePresence>
            {showTooltip && !isMobile && (
              <motion.div
                className="absolute bottom-full right-0 mb-3 bg-[#128c7e] text-white px-4 py-2 rounded-lg shadow-lg whitespace-nowrap"
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 10 }}
                transition={{ duration: 0.3 }}
              >
                <p className="font-poppins text-sm">
                  ¿Necesitas ayuda? ¡Escribinos!
                </p>
                <div className="absolute top-full right-4 w-0 h-0 border-l-8 border-r-8 border-t-8 border-transparent border-t-[#128c7e]" />
              </motion.div>
            )}
          </AnimatePresence>

          {/* Botón de WhatsApp */}
          <motion.button
            onClick={handleClick}
            onMouseEnter={() => !isMobile && setShowTooltip(true)}
            onMouseLeave={() => !isMobile && setShowTooltip(false)}
            className={`
              bg-gradient-to-br from-[#25d366] to-[#128c7e]
              text-white font-poppins font-semibold
              shadow-lg hover:shadow-[0_10px_30px_rgba(37,211,102,0.35)]
              border-2 border-white/20
              transition-all duration-300 ease-out
              flex items-center gap-3
              ${
                isMobile
                  ? "w-16 h-16 rounded-full justify-center"
                  : "px-6 py-3 rounded-full hover:-translate-y-1"
              }
            `}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            aria-label="Contactar por WhatsApp"
          >
            {/* Icono de WhatsApp */}
            <svg
              className={isMobile ? "w-8 h-8" : "w-6 h-6"}
              fill="currentColor"
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
            </svg>

            {/* Texto solo en desktop */}
            {!isMobile && <span className="text-base">¡Escribinos!</span>}
          </motion.button>

          {/* Pulse animation para llamar la atención */}
          {!showTooltip && (
            <motion.div
              className="absolute inset-0 bg-[#25d366] rounded-full"
              initial={{ scale: 1, opacity: 0.5 }}
              animate={{
                scale: [1, 1.2, 1],
                opacity: [0.5, 0, 0.5],
              }}
              transition={{
                duration: 2,
                repeat: Infinity,
                ease: "easeInOut",
              }}
              style={{ zIndex: -1 }}
            />
          )}
        </motion.div>
      )}
    </AnimatePresence>
  );
};

export default FloatingWhatsApp;
